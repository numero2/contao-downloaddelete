 ...
 * @package    Download Delete
 */
class ContentDownloadsDelete extends \ContentElement
{
	 ...
	 * @var string
	 */
	protected $strTemplate = 'ce_downloads_delete';
	...
	public function generate()
	{
		...
		if ($this->useHomeDir && FE_USER_LOGGED_IN)
		{
			...
			if ($this->User->assignDir && $this->User->homeDir)
			{
				...
				// delete file from userdir if given
				$delete = \Input::get('delete', true);

				if ($delete != '')
				{
					// get the file entries from the database
					$files = \FilesModel::findMultipleByIds($this->multiSRC);
					$path = $files->next()->path;

					// delete file
					if( file_exists(TL_ROOT . '/' . $path . '/' . $delete) ) {
						$delFile = new \File($path . '/' . $delete);
						$delFile->delete();
					}
				}
			}
	...
	protected function compile()
	{
		...
		while ($objFiles->next())
		{
			...
			else
			{
				...
				while ($objSubfiles->next())
				{
					...
					$strHref = \Environment::get('request');
					$strDelHref = \Environment::get('request');
					...
					if (preg_match('/(&(amp;)?|\?)delete=/', $strDelHref))
					{
						$strDelHref = preg_replace('/(&(amp;)?|\?)delete=[^&]+/', '', $strDelHref);
					}

					$strHref .= (($GLOBALS['TL_CONFIG']['disableAlias'] || strpos($strHref, '?') !== false) ? '&amp;' : '?') . 'file=' . \System::urlEncode($objSubfiles->path);
					$strDelHref .= (($GLOBALS['TL_CONFIG']['disableAlias'] || strpos($strDelHref, '?') !== false) ? '&amp;' : '?') . 'delete=' . \System::urlEncode($objFile->basename);
					...
					(
						...
						'name'      => $objFile->basename,
						'title'     => $arrMeta['title'],
						'titleDel'  => sprintf($GLOBALS['TL_LANG']['MSC']['downloads_delete'][0],$objFile->basename),
						'link'      => $objFile->basename,
						'caption'   => $arrMeta['caption'],
						'href'      => $strHref,
						'hrefDel'   => ($this->useHomeDir) ? $strDelHref : 'blah',
						'filesize'  => $this->getReadableSize($objFile->filesize, 1),
						'icon'      => TL_ASSETS_URL . 'assets/contao/images/' . $objFile->icon,
						'mime'      => $objFile->mime,
						'meta'      => $arrMeta,
						'extension' => $objFile->extension,
						'path'      => $objFile->dirname,
						'cnfrmDel'	=> sprintf($GLOBALS['TL_LANG']['MSC']['downloads_delete'][2],$objFile->basename)
					);
		...
		$this->Template->delete = $GLOBALS['TL_LANG']['MSC']['downloads_delete'][1];
